...
public:
    ...
    template <typename Dtype>
    int find_negatives(Blob<Dtype>& prob,
...
// OLHM -> on line hard mining
template <typename Dtype> class SoftmaxWithLossOLHMLayer : public LossLayer<Dtype>
{
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
  ...
  explicit SoftmaxWithLossOLHMLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param)
    {
    }

    // destruction function
    ~SoftmaxWithLossOLHMLayer()
    {
    }
 ...
 protected:
   ...
   * if propagate_down[1] is set.
   *
   * @param [top_output_Blob_vector] (length 1), providing the error gradient with
   *      respect to the outputs
   *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
   *      This Blob's diff will simply contain the loss_weight* @f$ \lambda @f$,
   *      as @f$ \lambda @f$ is the coefficient of this layer's output
   ...
   *      respect to the labels.
   *
   * @param [bottom_input_Blob_vector] (length 2)
   *        v[0]:predict v[1]:labels
   *   -# @f$ (N \times C \times H \times W) @f$
   *      the predictions @f$ x @f$; Backward computes diff
   *      @f$ \frac{\partial E}{\partial x} @f$
   *   -# @f$ (N \times 1 \times 1 \times 1) @f$
